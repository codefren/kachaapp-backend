...
from proveedores.models import Provider, Product, PurchaseOrder, PurchaseOrderItem
...
class ProveedoresAPITests(APITestCase):
    ...
    def test_create_and_retrieve_purchase_order(self):
        url = "/proveedores/api/purchase-orders/"
        payload = {
        ...
        # Retrieve detail
        detail_url = f"/proveedores/api/purchase-orders/{po_id}/"
    ...
    def test_list_purchase_order_items(self):
        ...
        url = "/proveedores/api/purchase-order-items/"
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertGreaterEqual(len(res.data), 2)
