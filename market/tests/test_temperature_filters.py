...
def test_filter_temperature_by_date(auth_client):
    ...
    temp_today = TemperatureRecordFactory(
        refrigerator=fridge, date=today, temperature=2.0
    )
    temp_yesterday = TemperatureRecordFactory(
        refrigerator=fridge, date=yesterday, temperature=3.0
    )
...
def test_filter_temperature_by_date_range(auth_client):
    ...
    for i in range(5):
        ...
        temp = TemperatureRecordFactory(
            refrigerator=fridge, date=date, temperature=float(i)
        )
        temps.append(temp)
...
def test_filter_temperature_by_market_and_date(auth_client):
    """Verifica que se puedan filtrar temperaturas por mercado y fecha específica."""
    # Crear dos mercados diferentes
    market1 = MarketFactory()
    market2 = MarketFactory()
    ...
    # Crear temperaturas para market1 en diferentes fechas
    temp1_today = TemperatureRecordFactory(
        refrigerator=fridge1, date=today, temperature=2.0
    )
    temp1_yesterday = TemperatureRecordFactory(
        refrigerator=fridge1, date=yesterday, temperature=1.0
    )

    # Crear temperaturas para market2 en diferentes fechas
    temp2_today = TemperatureRecordFactory(
        refrigerator=fridge2, date=today, temperature=3.0
    )
    temp2_yesterday = TemperatureRecordFactory(
        refrigerator=fridge2, date=yesterday, temperature=4.0
    )

    # Filtrar por market1 y fecha de hoy
    response = auth_client.get(
        f"/api/temperature-records/?market={market1.id}&date={today}"
    )
    assert response.status_code == 200
    assert len(response.data) == 1
    assert response.data[0]["id"] == temp1_today.id
    assert response.data[0]["temperature"] == 2.0

    # Filtrar por market1 y fecha de ayer
    response = auth_client.get(
        f"/api/temperature-records/?market={market1.id}&date={yesterday}"
    )
    assert response.status_code == 200
    assert len(response.data) == 1
    assert response.data[0]["id"] == temp1_yesterday.id
    assert response.data[0]["temperature"] == 1.0

    # Filtrar por market2 y fecha de hoy
    response = auth_client.get(
        f"/api/temperature-records/?market={market2.id}&date={today}"
    )
    assert response.status_code == 200
    assert len(response.data) == 1
    assert response.data[0]["id"] == temp2_today.id
    assert response.data[0]["temperature"] == 3.0
